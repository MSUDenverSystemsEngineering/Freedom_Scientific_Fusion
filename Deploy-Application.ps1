...
Try {
	...
	[string]$appScriptDate = '6/24/2021'
	...
	If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
		...
		## Install Fusion 2021 silently
		$exitCode = Execute-Process -Path "$dirFiles\F2021.2105.8.400.exe" -Parameters "/Type Silent" -WindowStyle "Hidden" -PassThru -WaitForMsiExec
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
		...
		## Remove Desktop Shortcuts of Fusion 2021
		Remove-File -Path "$env:Public\Desktop\FSReader 3.0.lnk"
		Remove-File -Path "$env:Public\Desktop\Fusion 2021.lnk"
		Remove-File -Path "$env:Public\Desktop\JAWS 2021.lnk"
		Remove-File -Path "$env:Public\Desktop\ZoomText 2021.lnk"
		## Set LSHOST Environment Variable for Network Licensing
		[Environment]::SetEnvironmentVariable('LSHOST', 'VMWAS32', 'Machine')
